...
import { resetPasswordTenant } from "@/libs/fetch/tenant";
...
const FormikResetPassTenant: FC<ResetPassProps> = ({ token }) => {
  ...
  const onResetPass = async (
    ...
    setLoading(true);
    try {
      const res = await resetPasswordTenant(password, token);
...
export default FormikResetPassTenant;
